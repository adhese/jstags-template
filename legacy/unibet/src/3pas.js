(function () {
  ...
  var addClickTrackerToIframeSrc = function (tag, clickUrl) {
    if (tag.includes("index.html?") && typeof clickUrl !== undefined) {
      ...
      var tagWithClick = tag.replace(
        "index.html?",
        "index.html?b64click=" + b64clickUrl + "&"
      );
      return tagWithClick;
    } else {
      return tag;
  ...
  var account = adheseDiv.getAttribute("data-adhese-account");
  if (account == null) account = "";

  var slot = adheseDiv.getAttribute("data-adhese-slot");
  if (slot == null) slot = "";

  const time = new Date().getTime();

  var adUri =
    "https://ads-" + account + ".adhese.com/json/sl" + slot + "/?t=" + time;
  ...
  AdheseAjax.request({
    ...
    for (var i = result.length - 1; i >= 0; i--) {
      if (result[i].ext == "zip") {
        ...
        if (result[i] && result[i].trackedImpressionCounter)
          addTrackingPixel(result[i].trackedImpressionCounter);
        ...
        if (clickTag) {
          ...
          var thirdPartyClickTag =
            clickTag + `https://ads-${account}.adhese.com/raylene/`;
          toInsertInBody = toInsertInBody.replace(
            `https://ads-${account}.adhese.com/raylene/`,
            thirdPartyClickTag
          );
        }
        ...
        iframe.style =
          "border: none;outline: none;overflow: hidden;top:0px; left:0px; margin: 0; padding: 0;";
