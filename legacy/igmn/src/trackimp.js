(function(){
    ...
    const getConsentString = function() {
    	try {
	    	window.top.__tcfapi('getTCData', 2, function(data, success) {
	    		if (success && data && data.tcString) {
	    			return data.tcString	
	    		}
			})
    	} catch (e) {
    		return "";
    ...
    // search for adslot in script src query string params
    // should be implemented like <script src="https://pool-igmn.adhese.com/tag/trackimp.js?track=mylocation-myformat"></script>
    const url = new URL(document.currentScript.getAttribute('src'));
	const scriptParams = Object.fromEntries(url.searchParams)
    let slot = "";
    let consentString = getConsentString();
	if (scriptParams && scriptParams.track) slot = scriptParams.track;
    let adUri = "https://ads-igmn.adhese.com/json/sl" + slot + "/xt" + consentString +"/?t=" + new Date().getTime();
   ...
   if (slot) {
        ...
        console.log('Script tag needs a track query string parameter, like https://pool-igmn.adhese.com/tag/trackimp.js?track=mylocation-myformat')
   }
}).call(this);