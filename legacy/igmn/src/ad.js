(function() {
    ...
    var requestAd = function(tcData) {
        ...
        var b64Url = btoa(window.location.href);
        var adUri = "https://ads-igmn.adhese.com/json/sl" 
            + (adheseDiv.getAttribute("data-adhese-adunit")).toLowerCase().replace(/\//g,"_").replace(/\(/g,"_").replace(/\)/g,"_") 
            + "/dt" + deviceType
            + "/xt" + consentString
            + "/xf" + b64Url + "/?t="
            + new Date().getTime();
            ...
            for (var i = result.length - 1; i >= 0; i--) {
                if (result[i].ext == 'zip') {
                    ...
                    if (clickTag) {
                        ...
                        var thirdPartyClickTag = clickTag + 'https://ads-igmn.adhese.com/raylene/';
                        toInsertInBody = toInsertInBody.replace('https://ads-igmn.adhese.com/raylene/', thirdPartyClickTag);
                    }
                    ...
                    iframe.style = "border: none;outline: none;overflow: hidden;top:0px; left:0px; margin: 0 auto; padding: 0; display: block;"; // Adjusted styles
