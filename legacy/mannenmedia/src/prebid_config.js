...
var syncContext = function(option) {
    if (isArticleForContextSync) {
        if (adhese_debug) {
            console.log("ADHESE: connect to weborama and send context");
            console.log("ADHESE: context sync url: " + "https://ads-mannenmedia.adhese.com/usersync/reverse-sync?url=" + encodeURIComponent("https://dx.frontend.weborama.com/collect?dsp_id=7&eid={ADHESE_USER_ID}&touchpoint=46&url=" + encodeURIComponent(location.href)));
        }
...
var syncDigitalAudience = function () {
    ...
    var DA9910735 = ['autoblog.nl', 'groen7.nl', 'autojunk.nl', 'autowiki.nl', 'abhd.nl', 'autowereld.nl', 'autowereld.com', 'autorai.nl', 'zerauto.nl', 'motorrai.nl', 'finnik.nl', 'routenet.nl', 'rijtesten.nl', 'grandprix33.nl', 'racingnews365.nl', 'gpupdate.nl'];
    var DA9910736 = ['apparata.nl', 'falder.nl', 'parra.nu', 'neveroffside.nl', 'manly.nl', 'mnlx.nl', 'dutchcowboys.nl', 'mnlk.nl', 'dailybase.nl', 'fhm.nl'];
    var DA9910737 = ['fcupdate.nl', 'gpupdate.nl', 'vi.nl', 'voetbalflitsen.nl', 'voetbalflitsen.be', 'voetbal.nl', 'grandprix33.nl', 'racingnews365.nl', 'indeleiderstrui.nl', 'mylaps.nl'];

    var requiredIsPresent = function(element, position, array) {
        if (url.includes(element)) {
            if (array == DA9910735) {
                addTrackingPixel('https://target.socialaudience.nl/bakery/pix/v?cid=9910735&itid=100');
            }
            if (array == DA9910736) {
                addTrackingPixel('https://target.socialaudience.nl/bakery/pix/v?cid=9910736&itid=100');
            }
            if (array == DA9910737) {
                addTrackingPixel('https://target.socialaudience.nl/bakery/pix/v?cid=9910737&itid=100');
            }
            return true;
        }
        return false;
    }
    DA9910735.some(requiredIsPresent);
    DA9910736.some(requiredIsPresent);
    DA9910737.some(requiredIsPresent);
}
...
var adheseViewportDimensions = {width:0,height:0};

var defineAdUnits = function() {

    adheseViewportDimensions = getViewportDimensions();
    ...
    try {
        ...
        try {
            ...
            if (adhese_debug) {console.log("ADHESE: defining slots on gptadslots array skipped")};
            console.log(e);
            try {
                var domain = window.location.hostname;
                loadDomainAdUnits(domain);
            } catch (e) {
                if (adhese_debug) {console.log("ADHESE: loading slots failed for domain")};
            }
        }
    }
}
...
var createPrebidAdunitFromSlot = function(slot, adUnits, xiParam, xtParam, tlParam, customParam) {
    ...
    if(adhese_debug) {console.log('sizeFromAdunit 1:', sizeFromAdunit); console.log(sizeFromAdunit)};
    //fix for inconsistent naming on gaspedaal
    if(sizeFromAdunit[0] == ["/11692722/GASPEDAAL.NL/GASPEDAAL.NL_WEB_HOME_300"]) {
        if(adhese_debug) {console.log('sizeFromAdunit 2:', sizeFromAdunit)};
        sizeFromAdunit = ['300', '250'];
    } else if (sizeFromAdunit[0].indexOf("_MOB_")!=-1) {
...
/*        if (slot.getAdUnitPath() == '/11692722/RACINGNEWS365/RACINGNEWS365_WEB_ARTIKEL_728X90_PREMIUM_DOWN') {
            o.bids.push({
                bidder: 'teads',
                params: {
                    placementId: 117607,
            ...
            o.mediaTypes.banner.sizes = [[728,90]]
        }*/ 
...
var createAdUnits = function() {
    ...
    targetArray.push(['xw',adheseViewportDimensions.width]);
    targetArray.push(['xh',adheseViewportDimensions.height]);
    ...
    try {
        for (var t = 0; t < targetConfig.length; t++) {
            ...
            if(target.length < 1) {
                try {
                    //in case targeting is set on slot level instead of page level:
    ...
    if (definedSlots) {
        ...
        for (var prop in definedSlots) {
            ...
            if (!regexIgnore.test(prop)) {
                if (adheseViewportDimensions.width > 768 && regexWeb.test(prop)) {
                ...
                } else if (adheseViewportDimensions.width < 769 && regexMob.test(prop)) {
          ...
          consentManagement: {
            ...
            userSync: {
              userIds: [{
                ...
                filterSettings: {
                    iframe: {
                        bidders: ['adhese', 'teads'], // '*' means all bidders
                        filter: 'include'
                    }
                },
            },
...
var initAdserver = function(bids) {
    for (var i in bids) {
        for (var j in bids[i].bids) {
            ...
            try {
                bid.ad = bid.ad.replace("[publisher_win_price]",bid.cpm);
            } catch(err) {}
