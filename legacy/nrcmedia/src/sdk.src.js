...
Adhese.prototype.enableViewabilityTracking = function(target, settings) {
    ...
    if (typeof settings === "object" && settings !== null) {
        settings.inViewPercentage ? observerOptions.threshold.push(settings.inViewPercentage) : observerOptions.threshold.push(.5);
        ...
        observerOptions.threshold.push(.5);
        ...
        entries.forEach(function(entry) {
            ...
            if (entry.isIntersecting) {
                if (entry.intersectionRatio >= target.viewability.inViewPercentage && target.viewability.trackerTimeout > 0) {
                    ...
                    }, target.viewability.trackerTimeout * 1e3);
...
Adhese.prototype.checkPreviewList = function() {
    ...
    if (inUrl != "" && inUrl != null) previewAds = JSON.parse(inUrl); else if (inCookie != "" && inCookie != null) previewAds = JSON.parse(inCookie);
    this.previewFormats = {};
...
(function(win) {
    var NULL = null, TRUE = true, FALSE = false, LOAD = "load", ON_STR = "on", MSG = "message", UNLOAD = "un" + LOAD, ONUNLOAD = ON_STR + UNLOAD, ONMSG = ON_STR + MSG, ONLOAD = ON_STR + LOAD, DG = "__defineGetter__", DS = "__defineSetter__", DP = "__defineProperty__", W3C_ATTACH = "addEventListener", W3C_DETACH = "removeEventListener", IE_ATTACH = "attachEvent", IE_DETACH = "detachEvent", TOLOWERCASE = "toLowerCase", EXPAND_COMMAND = "exp-ovr", COLLAPSE_COMMAND = "collapse", ERROR_COMMAND = "error", NOTIFY_GEOM_UPDATE = "geom-update", NOTIFY_FOCUS_CHANGE = "focus-change", NOTIFY_EXPAND = "expand", NOTIFY_COLLAPSE = COLLAPSE_COMMAND, NOTIFY_COLLAPSED = NOTIFY_COLLAPSE + "d", NOTIFY_FAILURE = "failed", NOTIFY_READ_COOKIE = "read-cookie", NOTIFY_WRITE_COOKIE = "write-cookie", STATUS_COLLAPSED = NOTIFY_COLLAPSED, STATUS_EXPANDED = NOTIFY_EXPAND + "ed", STATUS_COLLAPSING = "collapsing", STATUS_EXPANDING = NOTIFY_EXPAND + "ing", OUR_TAG_CLS_NAME = "sf", MAX_MSG_WAIT_TIME = 4e3, DOM_WATCH_INTERVAL = 3e3, GUID_VALID_TIME = 3e4, OBJ = "object", d = win && win.document, par = win && win.parent, sf = win && win.$sf, lib = sf && sf.lib, env = sf && sf.env, lang = lib && lib.lang, ParamHash = lang && lang.ParamHash, dom = lib && lib.dom, iframes = dom && dom.iframes, msgclient_fb = dom && dom.msgclient_fb, isIE = env && env.isIE, _ue = win && win.unescape, _cstr = lang && lang.cstr, _cnum = lang && lang.cnum, _append = dom && dom.append, _tags = dom && dom.tags, _elt = dom && dom.elt, _purge = dom && dom.purge, _attach = dom && dom.attach, _detach = dom && dom.detach, _attr = dom && dom.attr, loaded = FALSE, is_expanded = FALSE, force_collapse = FALSE, is_registered = FALSE, init_width = 0, init_height = 0, sandbox_cb = NULL, pending_msg = NULL, geom_info = NULL, pos_meta = NULL, win_has_focus = FALSE, guid = "", host_cname = "", can_use_html5 = FALSE, frame_id = "", pos_id = "", err_msg_timer_id = 0, orphan_timer_id = 0, inline_handler_timer_id = 0, err_msgs = [], unload_handlers = [], render_params, render_conf, ie_old_attach, w3c_old_attach, ie_old_detach, w3c_old_detach;
    ...
    function _set_alignment(xn, yn) {
        var fcDiv = _elt("sf_align"), fcDivStyle = fcDiv.style, xTxt, yTxt, preTxt = "position:absolute;";
        if (xn) {
            xTxt = "right:0px;";
        } else {
            xTxt = "left:0px;";
    ...
    function expand(deltaXorDesc, deltaY, p) {
        var xn = FALSE, yn = FALSE, doAlign = FALSE, cmd_nm = p ? "exp-push" : EXPAND_COMMAND, cmd_str = [ "cmd=", cmd_nm, "&pos=", pos_id ], dx = 0, dy = 0, r, b, t, l, align_el, align_el_st, align_buffer;
    ...
    function inViewPercentage() {
        var iv = _cnum(geom_info && geom_info.self && geom_info.self.iv, -1, 0), tv;
        if (iv >= 0) {
...
(function(){
    ...
    var ads = new Array();
    var configuredSlots = window.adheseSlots[getDeviceType()];
    var adsToLazyLoad = {};
    ...
    var registerTargets = function() {
        ...
        try {
            if (adheseData && adheseData.consent) targetArray.push(['tl', [adheseData.consent ? 'all' : 'none']]);
            if (adheseData && adheseData.loggedIn) targetArray.push(['li', [adheseData.loggedIn]]);
            if (adheseData && adheseData.pageType) targetArray.push(['pt', [adheseData.pageType]]);
            if (adheseData && adheseData.articleId) targetArray.push(['ai', [adheseData.articleId]]);
            if (adheseData && adheseData.deviceType) targetArray.push(['dt', [getDeviceType()]]);
            if (adheseData && adheseData.section) targetArray.push(['sc', [adheseData.section]]);
            if (adheseData && adheseData.subsection) targetArray.push(['ss', [adheseData.subsection]]);
    ...
    //function to reload a slot after a specified delay, only if a user hasn't scrolled within that time window.
    var startRefillTimer = function(slot, delay) {
        if (debug) console.log('starting refill timer for', slot, delay)
        var timeInMs = delay * 1000;
        var cancelReload = function() {
            userInteractionDetected = true;
            clearTimeout(reloadSlot);
            if (debug) console.log('user interaction detected - not reloading slot')
            window.removeEventListener('scroll', cancelReload); //remove scroll listener, no longer needed
        };

        var reloadSlot = setTimeout(function(){
            if (!userInteractionDetected) {
                var el = document.getElementById(slot.divId);
                removeContainer(el, slot); //remove current ad
                configuredSlots = {}; //reset configuredSlots so we can use requestAds()
                configuredSlots[slot.location] = slot;
                ads = []; //clear ads array
                requestAds(); //fetch new ads
            }
        },timeInMs);

        if (!scrollListenerAdded) { //make sure we don't add a scroll listener for each refresh. 
            window.addEventListener('scroll', cancelReload) //if scrolling is detected, cancel reload.
            scrollListenerAdded = true;
        }
    };

    var setContainerDimensions = function(slot) { //set w/h of container to avoid page bouncing when reloading slot
    ...
    var createContainerAndCheckForRender = function(ad) {
        for ( var c in configuredSlots ) {
            if ( ad.slotName == configuredSlots[c].slotCode ) {  
                ...
                if (container) {
                    ...
                    if ((configuredSlots[c].showToLoggedInUsers && loggedIn) || (configuredSlots[c].showToLoggedOutUsers && !loggedIn)) { //check if this slot should be shown based on login status
                        if (debug) console.log('Rendering ad for' , configuredSlots[c].location, ' - User is logged in:', loggedIn, ', showToLoggedInUsers is ', configuredSlots[c].showToLoggedInUsers, 'showToLoggedOutUsers is', configuredSlots[c].showToLoggedOutUsers);
                        if (configuredSlots[c].lazyLoaded) { //then check if slot should be lazyloaded
                            adsToLazyLoad[ad.slotName] = ad; //add to lazyload object if so:
                            adsToLazyLoad[ad.slotName].rendered = false;
                        } else {
                            adhese.safeframe.render(ad.slotName); //if not lazyloaded, render instantly
                            adhese.helper.addTrackingPixel(ad.tracker); //add impression tracker
                            try {
                                if(configuredSlots[c].allowRefill && configuredSlots[c].refillDelay) {
                                    startRefillTimer(configuredSlots[c], configuredSlots[c].refillDelay);
                                    if (debug) console.log('starting refill Timer for', configuredSlots[c].location);
                                }
                            } catch(e) {
                                console.log(e)
                                if (debug) console.log('allowRefill or refillDelay not defined for slot', configuredSlots[c]);
                            }
                        }
                    } else {
                        if (debug) console.log('NOT rendering ad for', configuredSlots[c].location, ' - User is logged in:', loggedIn, ', showToLoggedInUsers is ', configuredSlots[c].showToLoggedInUsers, 'showToLoggedOutUsers is', configuredSlots[c].showToLoggedOutUsers);
                    }
                }
            }
        }
    ...
    var requestAds = function() {
        ...
        if(debug) console.log('configured slots for ', getDeviceType(), ':', configuredSlots);
        //create adhese slots from configured slots
        for (var x in configuredSlots) {
            ads.push(adhese.tag(configuredSlots[x].size, {
                'location': configuredSlots[x].location,
                'containerId': configuredSlots[x].divId
            }));
            configuredSlots[x].slotCode = configuredSlots[x].location + '-' + configuredSlots[x].size;
        };
        ...
        var response = AdheseAjax.request({
            ...
            for (var x in results) {
                var ad = results[x];
                createContainerAndCheckForRender(ad);
            };
            lazyLoadAds(); //start rendering ads added to lazyload queue
        });        
    }
