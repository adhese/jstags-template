...
AdheseGateway.sizeMapping = {
    'h1': {
        desktop: [[1800, 1000], [970, 70], [970, 90], [970, 250]],
        tablet: [728, 90],
        mobile: [320, 100],
    },
    'r1': {
        ...
        tablet: [[300, 250]],
        mobile: [[320, 240], [300, 250], [320, 400]],
    },
    'r2': { 
        desktop: [[300, 600], [300, 250]],
        tablet: [[300, 250]],
...
AdheseGateway.appendSyncIframe = function() {
    ...
    iframe.setAttribute("src", "https://user-sync.adhese.com/iframe/user_sync.html?account=adswag");
    if (document.body) {
...
AdheseGateway.assignDeviceSizes = function(slots) {
    ...
    for (var i = 0; i < slots.length; i++) {
        var slot = slots[i];
        slot.sizes = AdheseGateway.getSlotSizes(slot.slotType, AdheseGateway.deviceType);
    }
};

AdheseGateway.createAdContainer = function(containerId, parentId, width, height) {
...
AdheseGateway.handleAdheseResponse = function(response, slots, ArmpConfig) {
    ...
    for (var i = 0; i < ads.length; i++) {
        ...
        if (slot) {
            ...
            AdheseGateway.createAdContainer(ad.slotName, slot.containerId, ad.width, ad.height);
...
AdheseGateway.sendAdheseRequest = function(slots, ArmpConfig) {
    ...
    xhr.send(JSON.stringify(adhese.getRequestPayload(AdheseGateway.adhese.slots)));
}
...
AdheseGateway.createAdheseSlots = function(slots, ArmpConfig) {
    ...
    for (var i = 0; i < slots.length; i++) {
        ...
        var location = ArmpConfig.data.domain + '_' + ArmpConfig.data.pageType + '_' + slot.slotType;
        ...
        if (typeof divId === 'undefined' || typeof location === 'undefined' || typeof format === 'undefined') {
            AdheseGateway.logger('skipping slot: ' + divId + ' ' + location + ' ' + format );
            continue;
        }
        AdheseGateway.logger('create adhese slot: ' + divId + ' ' + location + ' ' + format );
...
(function() {
    ...
    var url = 'https://cmp.inmobi.com'
        .concat('/choice/', 'FCB3RuFD51EpZ', '/', host, '/choice.js?tag_version=V3');
    var uspTries = 0;
    var uspTriesLimit = 3;
    element.async = true;
