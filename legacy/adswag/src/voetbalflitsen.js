...
AdheseGateway.init = function (ArmpConfig) {
  ...
  // Handle dynamic slots with interval checking
  function initializeDynamicSlots() {
    const bodyElement = document.querySelector(".body");
    if (!bodyElement) {
      console.log("Body element not found, retrying...");
      return;
    }
    ...
    if (paragraphs.length === 0) {
      console.log("No paragraphs found, retrying...");
      return;
    }

    const startAfterParagraph = 1; // First ad after the 1st paragraph
    const interval = 3; // Every 3 paragraphs
    const maxAds = 3; // Maximum of 3 ads
    const stopAfterParagraph = paragraphs.length; // Stop after the last paragraph

    let adCount = 0;
    let dynamicSlots = [];

    for (
      let i = startAfterParagraph;
      i < paragraphs.length && adCount < maxAds && i < stopAfterParagraph;
      i += interval
    ) {
      console.log("Creating ad slot after paragraph: ", i);
      const adDiv = document.createElement("div");
      adDiv.className = "dynamic-ad";
      adDiv.id = `dynamic-ad-${adCount + 1}`;
      adDiv.style.height = "1px"; // Example height
      adDiv.style.margin = "1px 0"; // Example margin

      paragraphs[i].insertAdjacentElement("afterend", adDiv);

      // Create new ad slot for SDK
      let adSlot = {
        slotType: "r1", // Example slot type, change as needed
        containerId: `dynamic-ad-${adCount + 1}`,
        devices: ["desktop", "tablet", "mobile"],
      };
      dynamicSlots.push(adSlot);
      console.log("Added ad slot: ", adSlot);

      adCount++;
    }

    console.log("Dynamic Slots Created: ", dynamicSlots);

    // Separate request for dynamic slots
    function sendDynamicAdheseRequest(slots) {
      AdheseGateway.logger("sendDynamicAdheseRequest()");
      // Do POST request to adhese
      var xhr = new XMLHttpRequest();
      xhr.open("POST", "https://ads-adswag.adhese.com/json", true);
      xhr.setRequestHeader("Content-Type", "application/json");
      ...
      xhr.onload = function () {
        if (xhr.status === 200) {
          ...
          // Collapse the divs if request fails
          for (let slot of slots) {
            var adContainer = document.getElementById(slot.containerId);
            if (adContainer) {
              adContainer.style.height = "0px";
            }
          }
      ...
      xhr.send(JSON.stringify(adhese.getRequestPayload(slots)));
    }

    // Initialize Adhese and send request for dynamic slots
    try {
      for (let i = 0; i < dynamicSlots.length; i++) {
        let slot = dynamicSlots[i];
        let location =
          "voetbalflitsen.nl_dynamic_ad_" + slot.slotType + "_" + (i + 1);
        let format = AdheseGateway.findBiggestSize(
          slot.sizes ||
            AdheseGateway.getSlotSizes(
              slot.slotType,
              AdheseGateway.getDeviceType()
            )
        );
        let slotCode = location + "-" + format;
        slot.slotCode = slotCode;

        let ad = adhese.tag(format, {
          divId: slot.containerId,
          location,
          slotCode,
          parameters: {},
        });
      ...
      console.log("Initialized Dynamic Slots: ", dynamicSlots);
      sendDynamicAdheseRequest(dynamicSlots);
    } catch (error) {
      console.error("Error initializing Adhese for dynamic slots: ", error);
    }
  }

  // Interval to check for elements and initialize dynamic slots
  var dynamicSlotInterval = setInterval(function () {
    try {
      initializeDynamicSlots();
      clearInterval(dynamicSlotInterval);
    } catch (error) {
      console.error("Error initializing dynamic slots: ", error);
    }
