//unofficial adapter for DPGMedia including ID5, origin, originInstance and P161 fixes.
...
function adResponse(bid, ad) {
  ...
  const dimensions = determineDimensions(ad);
...
function determineDimensions(ad) {
  const sizeMapping = {
    'layer1_large': {
      w: 640,
      h: 480
  ...
  const dimensions = {w:ad.width,h:ad.height};
  if (!dimensions.w || dimensions.w==0) {
    if (ad.adType.indexOf('x')!=-1) {
      dimensions.w = Number(ad.adType.split('x')[0]);
      dimensions.h = Number(ad.adType.split('x')[1]);
    } else {
      dimensions.w = sizeMapping[ad.adType].w;
      dimensions.h = sizeMapping[ad.adType].h;
    }
  }  
  return dimensions;
}
